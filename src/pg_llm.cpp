...
static void add_session_message(const std::string& session_id,
    ...
    snprintf(sql, sizeof(sql),
             ...
             session_id.c_str(), role.c_str(), content.c_str());
    SPI_exec(sql, 0);
    SPI_finish();
}
...
Datum pg_llm_add_model(PG_FUNCTION_ARGS) {
    ...
    manager.register_model("chatgpt", []() { return std::make_unique<pg_llm::ChatGPTModel>(); });
    manager.register_model("deepseek", []() { return std::make_unique<pg_llm::DeepSeekModel>(); });
    manager.register_model("hunyuan", []() { return std::make_unique<pg_llm::HunyuanModel>(); });
    manager.register_model("qianwen", []() { return std::make_unique<pg_llm::QianwenModel>(); });
...
Datum pg_llm_create_session(PG_FUNCTION_ARGS) {
    ...
    if (PG_NARGS() > 1 && !PG_ARGISNULL(1)) {
        ...
        session_id = std::string(VARDATA_ANY(session_id_text),
                               VARSIZE_ANY_EXHDR(session_id_text));
    } else {
    ...
    snprintf(sql, sizeof(sql),
             ...
             session_id.c_str(), model_instance.c_str());
    SPI_exec(sql, 0);
    SPI_finish();
...
Datum pg_llm_chat_session(PG_FUNCTION_ARGS) {
    ...
    std::string session_id(VARDATA_ANY(session_id_text),
                          VARSIZE_ANY_EXHDR(session_id_text));
    std::string prompt(VARDATA_ANY(prompt_text),
                      VARSIZE_ANY_EXHDR(prompt_text));
...
Datum pg_llm_parallel_chat(PG_FUNCTION_ARGS) {
    ...
    deconstruct_array(model_names_array, ARR_ELEMTYPE(model_names_array), typlen, typbyval, typalign,
                      &elements, &nulls, &num_models);

    std::vector<std::string> model_names;
    for (int i = 0; i < num_models; i++) {
        if (!nulls[i]) {
            text* model_name_text = (text*)elements[i];
            model_names.push_back(std::string(VARDATA_ANY(model_name_text),
                                            VARSIZE_ANY_EXHDR(model_name_text)));
        }
    }
