...
std::vector<float> HunyuanModel::get_embedding(const std::string& text) {
  if (!is_ready()) {
  ...
  request_body["model"] = "hunyuan-embedding";
  ...
  // Make API request
  ResponseData response_data;
  CURLcode res = make_api_request("https://api.hunyuan.com/v1/embeddings",
                                  request_body_str,
                                  response_data);
