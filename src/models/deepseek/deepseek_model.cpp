...
DeepSeekModel::DeepSeekModel() : LLMInterface() {
}

DeepSeekModel::~DeepSeekModel() {
}

bool DeepSeekModel::initialize(const std::string& api_key, const std::string& model_config) {
  if (!curl_) {
    return false;
  }
  ...
  model_name_ = config.get("model_name", "deepseek-chat").asString();
  api_endpoint_ = config.get("api_endpoint", "https://api.deepseek.com/v1/chat/completions").asString();

  is_initialized_ = true;
  return true;
}

ModelResponse DeepSeekModel::chat_completion(const std::string& prompt) {
  std::vector<ChatMessage> messages = {{"user", prompt}};
  return chat_completion(messages);
}

ModelResponse DeepSeekModel::chat_completion(const std::vector<ChatMessage>& messages) {
  if (!is_ready()) {
...
std::string DeepSeekModel::get_model_name() const {
  return model_name_;
}

std::string DeepSeekModel::get_model_info() const {
  return "DeepSeek Chat Model - " + model_name_;
}

bool DeepSeekModel::is_ready() const {
  return is_initialized_ && curl_;
}

CURLcode DeepSeekModel::make_api_request(const std::string& endpoint,
...
std::vector<float> DeepSeekModel::get_embedding(const std::string& text) {
  if (!is_ready()) {
  ...
  request_body["model"] = "deepseek-embedding";
  ...
  // Make API request
  ResponseData response_data;
  CURLcode res = make_api_request("https://api.deepseek.com/v1/embeddings",
                                  request_body_str,
                                  response_data);
